...
REGISTER_OBSERVATION(jump_command)
{
    ...
    // 2026-09-15 デバッグ: 'j' が本当にポリシーまで届いているかを見るための一時的な出力。
    // (Anaguma 版のみ。go2 側には入れていない)
    if (keyboard->consume_jump_trigger())
    {
        std::cout << "[jump] trigger received, g_jump_cmd=" << g_jump_cmd << std::endl;
        if (g_jump_cmd == 0.0f)
        {
            g_jump_cmd = 1.0f;
            g_jump_time_since_trigger = 0.0f;
            g_jump_since_trigger_wall = 0.0f;
            std::cout << "[jump] command raised" << std::endl;
        }
    }
