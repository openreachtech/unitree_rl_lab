...
class RealGo2ObsEvaluator:
    def __init__(self, interface: str, lidar_topic: str, state_topic: str):
        ...
        # 履歴バッファの初期化 (POLICY_HISTORY_LENGTH = 3)
        self.history_len = 3
        # 初期状態はすべてゼロで埋める (q_historyは相対関節偏差)
    ...
    def build_observation(self, step_history: bool = False):
        ...
        with self.lock:
            ...
            dq_current = self.latest_dq.copy()
        ...
        obs_vector = np.concatenate([
            ...
            commands,           # 6-8 (3)
            q_hist_flat,        # 9-44 (12 * 3 = 36)
            dq_hist_flat,       # 45-80 (12 * 3 = 36)
            act_hist_flat,      # 81-116 (12 * 3 = 36)
            heightmap           # 117-303 (187)
        ]).astype(np.float32)
        
        return obs_vector, gyro, projected_gravity, commands, q_rel_current

    def run_no_gui(self):
        """GUIを表示せず、304次元の結合観測ベクトルをログ表示するモード"""
        print("[INFO] Starting in No-GUI mode. Logging observations. Press Ctrl+C to stop.")
        
        try:
            while True:
                # 観測値をビルド (この呼び出しのたびに履歴を1ステップ進める)
                obs_vector, gyro, proj_grav, cmds, j_pos = self.build_observation(step_history=True)
                ...
                print(f"結合観測ベクトルサイズ: {len(obs_vector)} 次元 (117 proprio + 187 extero)")
                print("--------------------------------------------------")
                ...
                print(f"4. 最新の関節偏差 (joint_pos_rel) [12]:\n   {j_pos[:3]} (FR)\n   {j_pos[3:6]} (FL)")
                
                # ハイトマップの分析 (インデックス117以降)
                valid_cells = np.sum(obs_vector[117:] != self.processor.unknown_fill)
                print("--------------------------------------------------")
                print(f"5. ハイトマップ有効数: {valid_cells}/{self.num_cells} セル")
                
                # 全データ確認用
                print("--------------------------------------------------")
                print(f"観測ベクトル前半部分 (117次元 - ハイトマップ以外):\n{obs_vector[:117]}")
                print("==================================================")
        ...
        try:
            while vis.poll_events():
                ...
                if time.time() - last_print_time >= 1.0:
                    obs_vector, gyro, proj_grav, cmds, _ = self.build_observation(step_history=True)
                    valid_cells = np.sum(obs_vector[117:] != self.processor.unknown_fill)
