#!/usr/bin/env bash
# Loop 49 -- raise the distance target so the objective stops clamping.
#
...
echo "=== [$(ts)] Loop 49 start: apex back as the v_z lever (0->40, target 0.50, gate off), distance reverted to 2.06/74, from model_29200, +1500 iters ==="
...
echo "=== [$(ts)] Loop 49 exited with code $? ==="
