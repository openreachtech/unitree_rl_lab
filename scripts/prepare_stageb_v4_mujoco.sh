...
RUN_NAME=2026-09-03_02-24-06          # the resumed v4 run (iter600 -> 3000)
...
if [ ! -s "${RUN_DIR}exported/policy.onnx" ]; then
  echo "!!! [$(ts)] export failed -- see EXPORT_stageB_v4.log"
  exit 1
fi
...
DISPLAY=:1 setsid nohup ./unitree_mujoco > "${REPO}/MUJOCO_stageB_v4.log" 2>&1 < /dev/null &
sleep 6

cd "${REPO}/deploy/robots/go2/build"
DISPLAY=:1 setsid nohup gnome-terminal --title="go2_ctrl (Stage B v4 最終)" -- \
  bash -c "./go2_ctrl --network lo; echo; echo '=== 終了。Enterで閉じる ==='; read" \
  > "${REPO}/GO2_CTRL_stageB_v4.log" 2>&1 < /dev/null &
sleep 6
