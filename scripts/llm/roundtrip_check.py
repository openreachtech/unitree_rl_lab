...
- ``duration_s`` / ``distance_m`` / ``angle_deg`` match within 20% or round to the same number

A request the robot cannot do is not clamped or declined here: the model emits what was asked and
the compiler absorbs it downstream, so output and reparse are compared the same way in every
category that has a program at all. Categories with an empty program on both sides (chitchat,
impossible, ambiguous) trivially pass.
...
def step_matches(want: dict, got: dict) -> str | None:
    ...
    # 「後ろ向いて」「一周して」 name no side, and turning either way obeys them, so a reparse that
    # came back as "any" accepts whichever side the program picked.
    if "any" not in (want.get("dir"), got.get("dir")) and want.get("dir") != got.get("dir"):
    ...
    for field in ("duration_s", "distance_m", "angle_deg"):
