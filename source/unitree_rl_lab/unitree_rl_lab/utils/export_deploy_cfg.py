...
def _rename_observation_preserve_order(observations: dict, old_name: str, new_name: str) -> None:
    ...
    velocity-task-specific and legitimately do not apply to other task families (the
    jump/dynamic tasks carry ``jump_command`` instead). ``train_and_aggregate.py`` passes
    ``--deploy-keyboard-commands`` unconditionally, so raising here hard-failed every
    non-velocity task right after environment setup, before a single iteration ran.
    """
