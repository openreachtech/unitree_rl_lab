...
# GO2 is small and this is an in-place gesture (not locomotion), so perturbations are gentler
# than the G1 dance task.
VELOCITY_RANGE = {
    "x": (-0.25, 0.25),
    "y": (-0.25, 0.25),
    "z": (-0.1, 0.1),
...
class CommandsCfg:
    ...
    motion = mdp.MotionCommandCfg(
        ...
        # python scripts/mimic/go2_dmp_to_npz.py -f path/to/robot_go2_primitives/shake_hand --output_fps 50
        motion_file=f"{os.path.dirname(__file__)}/shake_hand.npz",
...
class EventCfg:
    ...
    base_com = EventTerm(
        ...
        params={
            ...
            "com_range": {"x": (-0.01, 0.01), "y": (-0.02, 0.02), "z": (-0.02, 0.02)},
        },
    )
...
class RobotEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the GO2 shake-hand motion tracking environment."""
...
class RobotPlayEnvCfg(RobotEnvCfg):
    def __post_init__(self):
        ...
        # follow camera scaled to Go2's smaller body size (same style as the G1 follow shot)
        self.viewer.eye = (1.6, 1.6, 0.7)
