...
class CommandsCfgPhase2(CommandsCfg):
    jump = CommandsCfg().jump.replace(
        ...
        # 0.20m: the highest reliably achievable height (plateaus there across multiple
        ...
        state_file="logs/rsl_rl/go2_jump_phase2/jump_curriculum_state.json",
    )
...
class RobotEnvCfgPhase2(RobotEnvCfg):
    """Phase 2: unified assisted jump + backflip + sideflip (one motion sampled per env)."""
