...
# ``robots/go2/multimode_env_cfg.py`` and the dims declared below can never drift
# apart -- same convention as the single-stance ``rsl_rl_ppo_cfg.py``.
PROPRIO_HISTORY_LENGTH = 4

# ang_vel(3) + projected_gravity(3) + velocity_commands(3) + gait_mode(3)
# + joint_pos_rel(12) + joint_vel_rel(12) + last_action(12) -- the single-stance
# biped's 45-dim proprioceptive observation plus a one-hot gait-mode command
# (quad / hind-biped / front-biped) so one policy can condition on which stance
# it's currently asked to hold.
...
# BipedCritic "critic" obs group layout: current proprio (incl. gait_mode)
# + true lin_vel(3) + true com_cop(3) + joint_effort(12).
CRITIC_OBS_DIM = PROPRIO_TERM_DIM + 3 + 3 + 12


@configclass
class BipedMultimodePPORunnerCfg(BasePPORunnerCfg):
    """Same ``BipedActorCritic``/``BipedPPO`` as the single-stance biped tasks
    (``rsl_rl_ppo_cfg.py``), just sized for the extra one-hot gait-mode command."""
