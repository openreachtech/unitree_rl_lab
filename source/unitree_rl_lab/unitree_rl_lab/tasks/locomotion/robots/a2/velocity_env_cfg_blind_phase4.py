"""Phase 4: stepping over short free-standing walls.

The A2 counterpart of ``robots/go2/velocity_env_cfg_blind_phase4.py``. Continual
learning on top of Phase 3: where Phase 3 is stairs -- a continuous surface the robot
climbs -- these are isolated walls it has to lift a leg over and clear, with nothing to
stand on in between. That is the hardest thing a blind lineage is asked to do, because a
wall is invisible to proprioception until a foot hits it, so the GRU has to infer it
from the contact and the stumble.

Two things carried over from the Go2 phase, both load-bearing there:

  * the wall height scales with difficulty rather than being fixed. A first attempt at a
    fixed height saw terrain_levels collapse and never recover -- the robot never got
    past the wall at all, so the curriculum had nothing to ratchet on.
  * the gait fix: ``joint_deviation_hips`` penalises hip deviation during the swing
    ...
    and ``calf_flexion_clearance`` rewards the substitute strategy directly.
...
else. Its ``natural_flex`` and ``max_flex`` are joint angles in radians and so do *not*
scale with body size; ``lookahead_distance`` and ``max_obstacle_height`` are lengths and
do, x1.29.
"""
...
from unitree_rl_lab.tasks.locomotion.robots.a2.velocity_env_cfg_blind_phase3 import (
...
PHASE4_TERRAIN_CFG = terrain_gen.TerrainGeneratorCfg(
    ...
    # the solid wall because Phase 3 here is plain stairs and this policy has never met a
    # floating tread. Rows stay at 10 for the difficulty ratchet.
    num_cols=3,
    num_rows=10,
    ...
    use_cache=False,
    sub_terrains={
        # Go2's (0.05, 0.25) height, (0.15, 0.05) thickness, 0.60 spacing, 2.0 platform
        # and 1.0 border, all x1.29. Height scales with difficulty for the reason in the
        # module docstring; thickness narrows over the same range.
        "thin_wall": terrains.MeshThinWallTerrainCfg(
            proportion=2.0,
            wall_height_range=(0.065, 0.32),
            wall_thickness_range=(0.19, 0.065),
            wall_spacing=0.77,
            platform_width=2.6,
            border_width=1.3,
        ),
...
class RewardsCfgPhase4(RewardsCfgPhase3BalanceMatched):
    """Phase 3's rewards plus the Go2 lineage's Phase 4 gait fix.
    ...
    The two posture penalties go the other way, back to the looser values. Phase 3 here
    inherits them from ``RewardsCfgPhase3BalanceMatched``, which raised them only so two
    reward designs could be compared on equal terms -- a measurement decision with no
    bearing on this phase. Clearing a wall means pitching the body and moving it
    vertically, which is exactly what these two charge for.
    """
    ...
    calf_flexion_clearance = RewTerm(
        ...
        params={
            ...
            "roughness_ref": 0.065,
            # Radians. A knee bend of a given angle lifts a 1.29x longer foot 1.29x
            # higher on its own, so these stay at Go2's values -- scaling them would
            # ask A2 for 1.29x the *angle* on top of that.
            "natural_flex": 0.05,
            "max_flex": 0.4,
        },
    )
...
class RobotEnvCfgPhase4(RobotEnvCfgPhase3BalanceMatched):
    ...
    scene: RobotSceneCfgPhase4 = RobotSceneCfgPhase4(num_envs=4096, env_spacing=3.2)
    rewards: RewardsCfgPhase4 = RewardsCfgPhase4()


# ---------------------------------------------------------------------------
# Play: one column per wall type at four difficulties, so a row shows the same wall
...
# wall_height_range is set so the four bands *centre* on Go2's 10 / 15 / 20 / 25 cm
# x1.29, i.e. 12.9 / 19.4 / 25.8 / 32.3 cm.
#
# Thickness is pinned rather than narrowing with difficulty, so height is the only thing
# that changes down a column -- and pinned at the training mix's hardest value, so every
...
PLAY_TERRAIN_CFG_PHASE4 = PHASE4_TERRAIN_CFG.replace(
    ...
    sub_terrains={
        name: cfg.replace(
            wall_height_range=(0.097, 0.355),
            wall_thickness_range=(0.065, 0.065),
        )
...
class RobotPlayEnvCfgPhase4(RobotEnvCfgPhase4):
    scene: RobotSceneCfgPhase4 = RobotSceneCfgPhase4(num_envs=32, env_spacing=3.2)

    def __post_init__(self):
        super().__post_init__()
        self.scene.num_envs = 32
        # .copy() rather than mutating PHASE4_TERRAIN_CFG's sub_terrains dict, which is
