...
def floating_thin_wall_terrain(
    ...
    Ported from ``feat/lidar``'s blind Go2 Phase 4 (wall crossing): mirrors how
    :func:`floating_pyramid_stairs_terrain` hollows out :func:`pyramid_stairs_terrain`'s
    solid risers, keeping only a thin tread at the top of each step. Here there's just
    one "step" per ring (the wall), so the whole solid box (z=0 to wall_height) is
    replaced by a thin slab whose top surface still sits at wall_height -- the "upper
    flat area" the solid wall used to have at its top, with the solid body beneath it
    removed. Used for the ``floating_thin_wall`` sub-terrain alongside the solid
    ``thin_wall`` type, so a trailing leg can be caught by a gap it cannot feel from
    above, the way it can with the floating pyramid-stairs treads.
    """
    wall_height = _lerp(cfg.wall_height_range, difficulty)
    wall_thickness = _lerp(cfg.wall_thickness_range, difficulty)
    wall_spacing = cfg.wall_spacing
