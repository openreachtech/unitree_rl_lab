...
# (example/isaaclab/isaaclab/sensors/ray_caster/patterns/patterns.py, livox additions only).
# Lives in unitree_rl_lab so the IsaacLab checkout stays pristine; the RayCaster only
# calls ``pattern_cfg.func`` so patterns work from any package. The .npy path resolution
# below falls back to ``scan_patterns/`` next to this file, which is where mid360.npy is.
